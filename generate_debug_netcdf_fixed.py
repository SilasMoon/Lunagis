...
def generate_debug_file():
    ...
    print(f"--- Generating Debug Pattern (FIXED) ---")
    ...
    # FIXED: Calculate number of pixels, then use linspace for exact centers
    width = int((max_x - min_x) / PIXEL_SIZE)
    height = int((max_y - min_y) / PIXEL_SIZE)

    # Generate coordinates at pixel CENTERS
    # For x: from min_x + half_pixel to max_x - half_pixel
    # For y: from max_y - half_pixel to min_y + half_pixel (DECREASING)
    x_coords = np.linspace(min_x + PIXEL_SIZE/2, max_x - PIXEL_SIZE/2, width)
    y_coords = np.linspace(max_y - PIXEL_SIZE/2, min_y + PIXEL_SIZE/2, height)

    print(f"Grid: {width}x{height} pixels")
    print(f"X range: {x_coords[0]:.2f} to {x_coords[-1]:.2f} m (step: {x_coords[1]-x_coords[0]:.2f})")
    print(f"Y range: {y_coords[0]:.2f} to {y_coords[-1]:.2f} m (step: {y_coords[0]-y_coords[1]:.2f})")
    print(f"Y is decreasing: {y_coords[0] > y_coords[-1]}")
    ...
    ds.title = "Debug Pattern (Fixed Pixel Centers)"
    ...
    print("\nKEY CHANGES:")
    print("1. Using np.linspace instead of np.arange for exact pixel centers")
    print("2. Coordinates now represent pixel CENTERS, not edges")
    print("3. All edge pixels are included (no off-by-one errors)")
